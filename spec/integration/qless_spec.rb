...
module Qless
  ...
  describe Qless::Client, :integration do
    ...
    def lua_helper(command, args)
      args.each { |x| lambda { lua_script([], [command, 12_345] + x) }.should raise_error }
    end
    ...
    describe '#events', :uses_threads do
      ...
      let!(:tracked  ) do
        job = client.jobs[q.put(Qless::Job, {foo: 'bar'})]
        job.track
        job
      end
      let!(:untracked) { client.jobs[q.put(Qless::Job, {foo: 'bar'})] }
    ...
    describe '#recur' do
      it 'can use recur in the most basic way' do
        ...
        Time.freeze
        q.recur(Qless::Job, { 'test' => 'test_recur_on' }, 1800)
      ...
      it 'gives the jobs it spawns with the same attributes it has' do
        ...
        # recurring job has
        Time.freeze
        q.recur(Qless::Job, { 'test' => 'test_recur_attributes' }, 100, priority: -10, tags: %w{foo bar}, retries: 2)
        q.pop.complete.should eq('complete')
        10.times.each do |i|
          Time.advance(100)
          job = q.pop
      ...
      it 'can get a recurring job' do
        ...
        Time.freeze
        jid = q.recur(Qless::Job, { 'test' => 'test_recur_get' }, 100, priority: -10, tags: %w{foo bar}, retries: 2)
        job = client.jobs[jid]
        job.class.should      eq(Qless::RecurringJob)
        job.priority.should   eq(-10)
      ...
      it 'can update tags for the recurring job appropriately' do
        ...
        Time.freeze
        jid = q.recur(Qless::Job, { 'test' => 'test_change_tags' }, 1, tags: %w{foo bar})
        q.pop.complete.should eq('complete')
        Time.advance(1)
        q.pop.tags.should eq(%w{foo bar})
        ...
        client.jobs[jid].tags.should eq(%w{bar foo hey})
        Time.advance(1)
        q.pop.tags.should eq(%w{bar foo hey})
      end
    ...
    context 'when there is a max concurrency set on the queue' do
      ...
      it 'can still timeout the jobs' do
        ...
        # But now it can't pop another one ...but it should still be able to
        # peek
    ...
    describe '#put' do
      ...
      it 'can put, peek, and pop many' do
        ...
        jids = 10.times.map { |x| q.put(Qless::Job, { 'test' => 'push_pop_many', 'count' => x})  }
      ...
      it 'can handle priority correctly' do
        ...
        jids = 10.times.map { |x| q.put(Qless::Job, { 'test' => 'put_pop_priority', 'count' => x}, priority: x) }
        last = jids.length
    ...
    describe '#move' do
      ...
      it 'expires locks when moved' do
        ...
        expect do
          job.heartbeat
        end.to raise_error(Qless::LuaScriptError, /not currently running/)
      end
      ...
      it 'moves non-destructively' do
        ...
        #   5) Compare 2 and 4
        jid = q.put(Qless::Job, { 'test' => 'move_non_destructive' }, tags: %w{foo bar}, priority: 5)
        before = client.jobs[jid]
        before.move('other')
        after  = client.jobs[jid]
        before.tags.should     eq(%w{foo bar})
    ...
    describe '#fail' do
      ...
      it "can't fail a canceled/expired job" do
        ...
        expect do
          bjob.fail('foo', 'bar')
        end.to raise_error(Qless::LuaScriptError, /does not/)
      end
    ...
    describe '#locks' do
      it 'invalidates locks after they expire' do
        ...
        expect do
          ajob.heartbeat
        end.to raise_error(Qless::LuaScriptError, /given out to another/)
      end
    ...
    describe '#cancel' do
      ...
      it 'can cancel a job, and prevent heartbeats' do
        ...
        expect do
          job.heartbeat
        end.to raise_error(Qless::LuaScriptError, /does not exist/)
        expect do
          job.complete
        end.to raise_error(Qless::Job::CantCompleteError, /does not exist/)
        client.jobs[ jid].should eq(nil)
      end
    ...
    describe '#complete' do
      ...
      it 'can allows a job to be completed only by the current worker' do
        ...
        expect do
          ajob.complete
        end.to raise_error(Qless::Job::CantCompleteError)
    ...
    describe '#expiration' do
      it 'can expire job data subject to a job expiration policy' do
        ...
        jids = 20.times.map { |x| q.put(Qless::Job, { 'test' => 'job_time_expiration', 'count' => x})  }
    ...
    describe '#stats' do
      it 'can correctly track wait times' do
        ...
        start = Time.now
        Time.stub!(:now).and_return(start)
        jids = 20.times.map do |x|
          q.put(Qless::Job, { 'test' => 'stats_waiting', 'count' => x})
        end
        ...
        expect(
          stats['run' ]['histogram'].reduce(0, :+)
        ).to eq(stats['run' ]['count'])
        expect(
          stats['wait']['histogram'].reduce(0, :+)
        ).to eq(stats['wait']['count'])
      end
      ...
      it 'can correctly track retries' do
        ...
        client.config['heartbeat'] = -10
        q.pop
        q.stats['retries'].should eq(0)
        q.pop
        q.stats['retries'].should eq(1)
      end
      ...
      it 'can update stats for the original day with respect to failures' do
        ...
        job = client.jobs[q.put(
          Qless::Job, { 'test' => 'stats_failed_original_day' })]
        job = q.pop
        job.fail('foo', 'bar')
        stats = q.stats
    ...
    describe '#retries' do
      ...
      it 'can reset the number of remaining retries when completed / moved' do
    ...
    describe '#workers' do
      ...
      it 'removes deregistered workers' do
        ...
        end.to change { registered_worker_names }.from([q.worker_name]).to([])
      end
    ...
    describe '#jobs' do
      it 'lets us peek at jobs in various states in a queue' do
        ...
        jids = 20.times.map do |i|
          q.put(Qless::Job, { 'test' => 'rssd' })
        end.to_set
        client.config['heartbeat'] = -60
        jobs = q.pop(20)
        ...
        jids = 20.times.map do |i|
          q.put(Qless::Job, { 'test' => 'rssd'}, delay: 60)
        end
        expect(
          (q.jobs.scheduled(0, 10) + q.jobs.scheduled(10, 10)).to_set
        ).to eq(jids.to_set)

        jids = 20.times.map do |i|
          q.put(Qless::Job, { 'test' => 'rssd'}, depends: jids)
        end
        expect(
          (q.jobs.depends(0, 10) + q.jobs.depends(10, 10)).to_set
        ).to eq(jids.to_set)
      end

      it 'does not list now-ready jobs in scheduled' do
        Time.freeze
    ...
    describe '#retry' do
      ...
      it 'fails when we exhaust its retries through retry()' do
        q.put(Qless::Job, { 'test' => 'test_retry_fail' }, retries: 2)
        client.jobs.failed.should eq({})
      ...
      it 'prevents us from retrying jobs not running' do
        ...
        expect do
          job.retry
        end.to raise_error(Qless::LuaScriptError, /given to another/)
      ...
      it 'stops reporting a job as being with a worker when is retried' do
        jid = q.put(Qless::Job, { 'test' => 'test_retry_workers' })
        job = q.pop
        client.workers[Qless.worker_name].should eq(
          { 'jobs' => [jid], 'stalled' => {} })
    ...
    describe '#tag' do
      ...
      it 'can tag in the most basic way' do
        ...
        job.tag('foo')
        client.jobs.tagged('foo').should eq(
          { 'total' => 1, 'jobs' => [job.jid] })
        client.jobs.tagged('bar').should eq(
          { 'total' => 0, 'jobs' => {} })
        job.tag('bar')
        client.jobs.tagged('foo').should eq(
          { 'total' => 1, 'jobs' => [job.jid] })
      ...
      it 'can tag a job when we initially put it on' do
        client.jobs.tagged('foo').should eq({ 'total' => 0, 'jobs' => {} })
        client.jobs.tagged('bar').should eq({ 'total' => 0, 'jobs' => {} })
        jid = q.put(Qless::Job, { 'test' => 'tag_put' }, tags: %w{foo bar})
      ...
      it 'can return the top tags in use' do
        # 1) Make sure that it only includes tags with more than one job
        #    associated with it
    ...
    describe '#dependencies' do
      it 'can recognize dependencies' do
        ...
        job = q.pop
        jid = q.put(Qless::Job,
                    { 'test' => 'depends_put' }, depends: [job.jid])
        q.pop.should eq(nil)
        ...
        jids = 10.times.map do |i|
          q.put(Qless::Job, { 'test' => 'depends_put' })
        end
      ...
      it 'can add dependencies at completion, too' do
        ...
        jids = 10.times.map do |i|
          q.put(Qless::Job, { 'test' => 'depends_complete' })
        end
        jid  = job.jid
      ...
      it 'deals with cancelation well with dependencies' do
        ...
        a = q.put(Qless::Job, { 'test' => 'depends_canceled' })
        q.put(Qless::Job, { 'test' => 'depends_canceled' }, depends: [a])
        expect do
          client.jobs[a].cancel
        end.to raise_error(/is a dependency/)
      end
      ...
      it 'can bulk cancel a dependency graph of jobs regardless of ordering' do
        jids = create_dep_graph
        expect { client.bulk_cancel(jids) }.to change { q.length }.to(0)

        jids = create_dep_graph
        expect do
          client.bulk_cancel(jids.reverse)
        end.to change { q.length }.to(0)
      end

      it 'cannot cancel a set of jids that extra dependencies' do
        a = q.put(Qless::Job, { 'test' => 'depends_canceled' })
        b = q.put(Qless::Job, { 'test' => 'depends_canceled' }, depends: [a])
        c = q.put(Qless::Job, { 'test' => 'depends_canceled' })
        q.put(Qless::Job, { 'test' => 'depends_canceled' }, depends: [c, a])

        expect do
          client.bulk_cancel([a, b, c])
        end.to raise_error(/is a dependency/)
      end

      it 'ignores unknown jids given to bulk_cancel' do
        jid_1, jid_2 = create_dep_graph
        jids = ['not_a_real_jid_1', jid_1,
                'not_a_real_jid_2', jid_2,
                'not_a_real_jid_3']
      ...
      it 'unlocks a job only after dependencies have completely finished' do
      ...
      it 'can support dependency chains' do
        ...
        10.times do |i|
          jids.push(q.put(Qless::Job,
                          { 'test' => 'cascading_dependency' },
                          depends: [jids[i]]))
        end
      ...
      it 'supports removing dependencies' do
        ...
        jids = 10.times.map do |i|
          q.put(Qless::Job, { 'test' => 'remove_dependency' })
        end
        ...
        end.to raise_error(/not in the depends state/)
        job = q.pop
        expect do
          job.undepend(b)
        end.to raise_error(/not in the depends state/)
      ...
      it 'lets us see dependent jobs in a queue' do
        ...
        # When we remove a dependency, we should no longer see that job as a
        # dependency
        client.jobs[b].undepend(a)
    ...
    describe '#pause' do
      it 'stops the given queue from being processed until #unpause' do
    ...
    describe '#lua' do
      ...
      it "checks put's arguments" do
        lua_helper('put', [
          ...
          %w{deadbeef foo []},
          # Non-dictionary data
          %w{deadbeef foo "foobar"},
          # Malformed delay
          %w{deadbeef foo {} howdy},
          # Malformed priority
          %w{deadbeef foo {} 0 priority howdy},
          # Malformed tags
          %w{deadbeef foo {} 0 tags [\}},
          # Malformed retries
          %w{deadbeef foo {} 0 retries hello},
          # Mutually exclusive options
          %w{deadbeef foo {} 5 depends ["hello"]}
        ])
      end
      ...
      it "checks recur's arguments" do
        lua_helper('recur.on', [
          ...
          %w{testing foo.klass},
          %w{testing foo.klass {}},
          %w{testing foo.klass {} 12345},
          %w{testing foo.klass {} 12345 interval},
          %w{testing foo.klass {} 12345 interval 12345},
          %w{testing foo.klass {} 12345 interval 12345 0},
          # Malformed data, priority, tags, retries
          %w{testing foo.klass [\} 12345 interval 12345 0},
          %w{testing foo.klass {} 12345 interval 12345 0 priority foo},
          %w{testing foo.klass {} 12345 interval 12345 0 retries foo},
          %w{testing foo.klass {} 12345 interval 12345 0 tags [\}},
        ])
        ...
        lua_helper('recur.update', [
          %w{update},
          %w{update priority foo},
          %w{update interval foo},
          %w{update retries foo},
          %w{update data [\}},
        ])
      end
